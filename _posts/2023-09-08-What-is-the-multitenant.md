...
주관적인 의견을 덧붙여서 아키텍쳐 방향에 대해서 설명하고자 합니다.

## 왜 데이터베이스 아키텍쳐가 변경된것일까요? 
...
오라클 데이터베이스는 두가지 방법을 제공하고 있는것 같습니다. 
...
오라클은 다음과 같은 관점으로 데이터베이스 운영을 권장하는 것으로 보입니다.
> "애플리케이션은 Kubernetes환경에서 운영하고 데이터베이스는 Multitenant 환경에서 운영하세요"

## Multitenant 아키텍쳐가 갖는 방향성은 무엇일까요?
...
데이터베이스를 모두 통합하려고 하려고 시도하는것보다는 접근하는 전략이 중요할것 같습니다. 
대고객 업무나 매우 중요한 업무는 여러개의 DB와 통합되어 운영하지 말고 하나의 DB로 운영하는것이 더 나을수 있습니다.  왜냐하면 DB 운영 시 첫 번째, 두 번째로 고려하는 것이 안정성이기 때문입니다. 안정적인 환경을 유지하기 위해서는 많은 자원과 독립적인 운영환경이 필요합니다. 
...
지금 시간이 있을때 좀더 일찍 준비한다면 더 나은 운영환경을 만들수 있지 않을까 싶습니다. 
